...
Runs end-to-end data pipeline: web scraping, SQLite database storage,
SQL execution, pandas integration, and automated validation.
"""
...
# Add data_pipeline folder to Python path
...
def export_outputs(query_results: dict, validation_results: dict, df_scraped: pd.DataFrame) -> None:
    """Save scraped dataset CSV, SQL query results report, and validation summary JSON."""
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    # 1. Save CSV
    csv_path = os.path.join(OUTPUT_DIR, "scraped_books_dataset.csv")
    df_scraped.to_csv(csv_path, index=False)
    print(f"[Output] Dataset saved to '{csv_path}'")

    # 2. Save SQL Query Markdown Report
...
def run_pipeline() -> bool:
    """Execute complete Module 1 Data Pipeline workflow."""
    print("==================================================")
    print("STARTING MODULE 1: ZEPTO DATA PIPELINE EXECUTION")
    print("==================================================")

    try:
        # Step 1: Web Scraping & Cleaning
        ...
        print("\n--- STEP 5: Exporting Output Reports ---")
